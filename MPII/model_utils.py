...
from CONSTANTS import SKELETON, keypoint_labels, used_kps
...
def calculate_heatmap_optimized(kp_id, keypoints, pad, heatmap_ps_map):
    H,W = 368+ pad*2, 368+ pad*2
    ps = int(heatmap_ps_map['ps'])
    g_vals = heatmap_ps_map['ps_vals'].copy()
    cond = np.invert(np.logical_and(keypoints[:,kp_id,0]==0, keypoints[:,kp_id,1]==0))
    
    ps_hf = ps//2
    points = keypoints[:,kp_id, :2][cond]
...
def get_heatmap_masks_optimized(keypoints, sigma, heatmap_ps_map, kp_ids = KEYPOINT_ORDER):
    h,w = 368, 368
    pad = 13
    ...
    cond = np.invert(np.logical_and(kps_copy[:,:,0]==0, kps_copy[:,:,1]==0))
    kps_copy[:,:,:2][cond] = kps_copy[:,:,:2][cond]+pad
    
    for i, kp_id in enumerate(kp_ids):
        mask, HM_IS_LABELED = calculate_heatmap_optimized(kp_id, kps_copy, pad, heatmap_ps_map)
...
def calculate_paf_mask_optimized(joint_pair, keypoints, paf_sz, limb_width):
...
def get_paf_masks_optimized(keypoints, limb_width, part_pairs=SKELETON):
    paf_sz = 46
    ...
    kps_copy[:,:,:2][cond] = kps_copy[:,:,:2][cond]*downsample_ratio
    
    for i, joint_pair in enumerate(part_pairs):
        mask, PAF_IS_LABELED = calculate_paf_mask_optimized(joint_pair, kps_copy, paf_sz, downsample_ratio*limb_width)
...
def get_keypoints_from_annotations(ann):
    return ann["all_kps"]
...
def paf_and_heatmap_loss(res, pafs_gt, paf_inds, hms_gt, hm_inds):
    cumulative_paf_loss = 0
    cumulative_hm_loss = 0
   
    cumulative_paf_loss = torch.dist(res[1][paf_inds], pafs_gt[paf_inds]) + torch.dist(res[3][paf_inds], pafs_gt[paf_inds])
    
    scaled_hms_0 = F.interpolate(res[0], 368, mode="bilinear", align_corners=True).to(device)
    scaled_hms_2 = F.interpolate(res[2], 368, mode="bilinear", align_corners=True).to(device)

    cumulative_hm_loss += torch.dist(scaled_hms_0[hm_inds], hms_gt[hm_inds]) + torch.dist(scaled_hms_2[hm_inds], hms_gt[hm_inds])
...
def get_heatmap_ps_map():
    heatmap_ps_map = {
            'ps' : 25,
            'ps_vals' : GAUSSIAN_25x25
    }
